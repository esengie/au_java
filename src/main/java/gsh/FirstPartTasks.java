...
public final class FirstPartTasks {
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(
                (lhs, rhs) -> avgRating(rhs.getTracks().stream())
                        .compareTo(avgRating(lhs.getTracks().stream()))
        ).collect(Collectors.toList());
    }
    ...
    private static int maxRating(Stream<Track> track) {
        Optional<Track> max = track.max((lhs, rhs) -> lhs.getRating() - rhs.getRating());
        return max.isPresent() ? max.get().getRating() : 0;
    }

    private static Double avgRating(Stream<Track> track) {
        OptionalDouble avg = track.mapToInt(Track::getRating).average();
